...
@Component({
  ...
  template: `
    ...
    <div class="notif-toast" *ngIf="toastVisible"
         [class.notif-toast-chat]="toastNotif?.type === 'CHAT'"
         (click)="onToastClick()">
  ...
  styles: [`
    ...
    .bell-btn {
      ...
      border-radius: 7px; color: #a0a0a0; cursor: pointer; transition: all 0.15s;
    }
    ...
    .badge {
      ...
      background: #e05555; color: #fff; font-size: 9px; font-weight: 700;
      min-width: 16px; height: 16px; border-radius: 8px;
      display: flex; align-items: center; justify-content: center;
      padding: 0 3px; border: 1.5px solid #0a0a0a; animation: popIn 0.2s ease;
    }
    @keyframes popIn {
    ...
    .notif-dropdown {
      ...
      box-shadow: 0 16px 48px rgba(0,0,0,0.6); z-index: 999; overflow: hidden;
    }
    .notif-header {
    ...
    .clear-btn {
      ...
      font-size: 11px; color: #606060; transition: color 0.12s;
    }
    .clear-btn:hover { color: #e05555; }
    .notif-empty { padding: 28px; text-align: center; font-size: 12px; color: #606060; }
    ...
    .notif-preview {
      font-size: 12px; color: #a0a0a0; line-height: 1.4;
    ...
    .notif-toast {
      ...
      max-width: 300px; animation: slideIn 0.25s ease; cursor: pointer;
    }
    .notif-toast:hover { background: #222; }
...
export class NotificationBellComponent implements OnInit, OnDestroy {
  ...
  @Output() openComment = new EventEmitter<number>();
  ...
  ngOnInit(): void {
    this.subs.push(
      this.notifService.savedNotifications$.subscribe(notifs => {
  ...
  onNotifClick(notif: NotificationDTO): void {
    ...
    if (notif.type === 'CHAT') {
      ...
      this.openComment.emit(notif.ticketId);
    }
  }
  ...
  onToastClick(): void {
    ...
    this.onNotifClick(this.toastNotif);
  }

  dismiss(index: number, event: MouseEvent): void {
    event.stopPropagation();
    const originalIndex = this.notifications.length - 1 - index;
    this.notifService.removeNotification(originalIndex);
  }

  clearAll(): void { this.notifService.clearSavedNotifications(); }
